...
void GateLamp::paint(juce::Graphics& g)
{
    ...
    // (section 5), independent of the lamp's own cosmetic closing decay.
    const bool open = processorRef.isGateOpen();
    g.setColour(open ? Colour::gateOpenLabelActive : Colour::gateOpenLabelInactive);
    // TODO(design): Barlow Condensed 700 not yet in design/assets/ - default bold sans placeholder
    // (see GatecrasherTheme::labelFontBold).
    g.setFont(labelFontBold(10.0f));
    g.drawText("GATE OPEN",
               juce::Rectangle<float>(Layout::gateOpenLabelX, Layout::gateOpenLabelY,
